...
BIRDEYE_HISTORICAL_PRICE_URL = "https://public-api.birdeye.so/defi/history_price"
...
def load_trade_cache(mint: str) -> List[str]:
    ...
    try:
        ...
        if isinstance(data, set):
            return list(data)
        if isinstance(data, pd.DataFrame) and "signature" in data.columns:
            return data["signature"].dropna().astype(str).tolist()
        return list(data)
    except Exception as e:
        logger.warning(f"Failed loading trade cache for {mint}: {e}")
        return []

def save_trade_cache(mint: str, signatures: Iterable[str]) -> None:
    path = _trade_cache_path(mint)
    uniq = sorted(set([s for s in signatures if s]))
    joblib.dump(uniq, path)
...
async def get_current_prices_multi(client: HttpClient, mints: List[str]) -> Dict[str, float]:
    ...
    try:
        ...
        # Expecting structure: data: { "<mint>": { value: ... }, ... } or similar; handle common shapes
        prices = {}
        if isinstance(data, dict):
            # shape might be {"<mint>": {"value": 1.23}, ...} or {"items":[...]}
            for k, v in data.items():
                if isinstance(v, dict) and v.get("value") is not None:
                    try:
                        ...
                        prices[k] = 0.0
        # fallback if data contains items list
...
async def get_flattened_trades(client: HttpClient, token_mints: List[str], limit: int = 100) -> pd.DataFrame:
    ...
    # Load cached signatures for each mint once at the beginning
    mint_cached_sigs = {mint: set(load_trade_cache(mint) or []) for mint in token_mints}
    
    for mint in token_mints:
        cache_path = _trade_cache_path(mint)
        ...
        if os.path.exists(cache_path):
            try:
                ...
                if isinstance(cached_data, pd.DataFrame):
                    cached_df = cached_data
                else:
                    cached_df = pd.DataFrame(cached_data) if cached_data else pd.DataFrame()
            except Exception as e:
                logger.warning(f"Failed loading DataFrame cache for {mint}: {e}")

            if "blocktime" in cached_df.columns and not cached_df.empty:
                try:
                    cached_df["blocktime"] = pd.to_datetime(cached_df["blocktime"], unit="s", utc=True, errors="coerce")
                    latest_time = cached_df["blocktime"].max()
                except Exception as e:
                    logger.warning(f"Error converting blocktime column for {mint}: {e}")
                    latest_time = None

        txs = await get_token_trades_by_address(client, mint, transaction_type="SWAP", page_size=limit, cached_signatures=mint_cached_sigs[mint])
        rows = []
        for tx in txs:
            ...
            # Additional check to ensure we get the right token
        ...
        if not new_df.empty:
            ...
            combined = combined.drop_duplicates(subset=["signature", "blocktime"], keep="first")
            
            # Explicitly cast dtypes before saving to prevent issues
            for col, dtype in {
                ...
                if col in combined.columns:
                    combined[col] = combined[col].astype(dtype, errors="ignore")
            
            try:
                joblib.dump(combined, cache_path)
                logger.info(f"💾 Saved {len(combined)} records to {cache_path}")
            except Exception as e:
                logger.warning(f"Failed saving cache to {cache_path}: {e}")
            
            mint_cached_sigs[mint].update(new_df["signature"].dropna().unique().tolist())
            save_trade_cache(mint, mint_cached_sigs[mint])
    ...
    if not trades_df.empty:
        # Final type casting for the combined DataFrame
...
async def get_trades_with_prices(client: HttpClient, token_mints: List[str], limit: int = 100) -> pd.DataFrame:
    ...
    enriched_dfs = []
    # Identify all unique tokens that were sold
    sold_mints_to_fetch = trades_df["token_sold_mint"].dropna().unique().tolist()
    
    # Batch historical price fetching to avoid redundant calls
...
def calculate_wallet_trade_metrics(trades_df: pd.DataFrame, wsol_mint: str = WSOL_MINT) -> List[Dict[str, Any]]:
    ...
    def get_avg_buy_price(group_df):
        ...
        buy_trades["total_amount_sold"] = buy_trades["amount_sold"]
...
async def get_wallet_networth_with_trades(
    ...
    for wallet_address in wallet_addresses:
        ...
        for t in tokens_iter:
            ...
            current_price = prices_map.get(mint) or await get_token_current_price(client, mint)
        ...
        # Calculate ROI based on total investment, not just total value
...
if __name__ == "__main__":
    ...
    async def main():
        # instantiate client with our default rate limits
        async with HttpClient(rate_limits=DEFAULT_RATE_LIMITS) as client:
            ...
            # Additional logic to handle if no wallets are passed
            if not wallets and not df.empty:
                # Find the most active trader and analyze their wallet

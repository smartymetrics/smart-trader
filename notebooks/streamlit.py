...
# Assuming the script is saved as 'smart_trader.py'
try:
...
def calculate_trader_metrics_fixed(results):
    """Calculate trader metrics with proper PnL and ROI calculations"""
    trader_summary = []
    ...
    for trader in results:
        ...
        for token_trade in token_breakdown:
            buy_vol = token_trade.get('buy_volume_usd', 0)
            sell_vol = token_trade.get('sell_volume_usd', 0)
            pnl = token_trade.get('pnl_usd', 0)
            status = token_trade.get('status', '')
            
            total_buy_volume += buy_vol
            total_sell_volume += sell_vol
            
            if status == 'Profit' or status == 'Loss':
                realized_pnl += pnl
            elif status == 'Unrealized':
                # For unrealized, we need current token value - cost basis
                # Since we only have buy volume, unrealized = current_value - buy_volume
                unrealized_pnl += pnl  # This should be calculated differently
            
            if status == 'Profit' or (status == 'Unrealized' and pnl > 0):
                tokens_in_profit += 1
        
        total_pnl = realized_pnl + unrealized_pnl
        
        # Portfolio Net Worth = Current holdings value (not trading volume)
        # For now, using total_sell_volume as proxy for realized value + remaining holdings
        portfolio_net_worth = total_buy_volume  # Cost basis of all investments
        
        # ROI based on total investment (buy volume)
        roi = (total_pnl / total_buy_volume * 100) if total_buy_volume > 0 else 0
        
        trader_summary.append({
            'Trader ID': truncate_address(trader_id),
            'Portfolio Net Worth': portfolio_net_worth,
            'Total PnL': total_pnl,
            'Realized PnL': realized_pnl,
            'Unrealized PnL': unrealized_pnl,
            'ROI (%)': roi,
            'Num Tokens in Profit': tokens_in_profit,
...
def main():
    ...
    if token_input_method == "Predefined Examples":
        example_tokens = {
            ...
            "Example Token 2": "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v",
            "Example Token 3": "Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB"
        }
...
def display_token_trade_analysis(results, token_addresses):
    """Display token trade analysis results with focus on key portfolio metrics"""
    ...
    # === Use fixed calculation function ===
    trader_summary = calculate_trader_metrics_fixed(results)
    ...
    total_portfolio_worth = sum(t['Portfolio Net Worth'] for t in trader_summary)
    ...
    total_tokens_in_profit = sum(t['Num Tokens in Profit'] for t in trader_summary)
    ...
    with col1:
        ...
        <div class="metric-card">
            <h4>💰 Portfolio Net Worth</h4>
            <h3>{format_usd(total_portfolio_worth)}</h3>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        st.metric("Total Profitable Positions", total_tokens_in_profit)
    ...
    display_df['Portfolio Net Worth'] = display_df['Portfolio Net Worth'].apply(format_usd)
    display_df['Total PnL'] = display_df['Total PnL'].apply(format_usd)
    display_df['Realized PnL'] = display_df['Realized PnL'].apply(format_usd)
    display_df['Unrealized PnL'] = display_df['Unrealized PnL'].apply(format_usd)
    ...
    if len(trader_summary) > 0:
        ...
        with col2:
            ...
            fig_roi = px.scatter(
                trader_df, 
                x='Num Tokens in Profit', 
                y='ROI (%)',
                size='Portfolio Net Worth',
                title="ROI vs Number of Profitable Tokens",
                color='Total PnL',
                color_continuous_scale='RdYlGn',
                hover_data=['Trader ID', 'Portfolio Net Worth']
            )
        ...
        with col2:
            # Portfolio Worth vs ROI
            fig_worth_roi = px.scatter(
                trader_df,
                x='Portfolio Net Worth',
                y='ROI (%)',
                title="Portfolio Net Worth vs ROI",
                color='Num Tokens in Profit',
                size='Total PnL',
                hover_data=['Trader ID'],
                color_continuous_scale='viridis'
            )
            fig_worth_roi.update_layout(xaxis_title="Portfolio Net Worth (USD)", yaxis_title="ROI (%)")
            st.plotly_chart(fig_worth_roi, use_container_width=True)
    ...
    with col3:
        st.write("**Top 5 by Portfolio Net Worth:**")
        top_worth = trader_df.nlargest(5, 'Portfolio Net Worth')[['Trader ID', 'Portfolio Net Worth']].copy()
        top_worth['Portfolio Net Worth'] = top_worth['Portfolio Net Worth'].apply(format_usd)
        st.dataframe(top_worth, hide_index=True, use_container_width=True)
...
def display_trader_portfolio_analysis(results, token_addresses):
    ...
    for wallet in results:
        with st.expander(f"Wallet: {truncate_address(wallet.get('wallet_address', ''))}"):
            col1, col2, col3 = st.columns(3)
            ...
            with col3:
                ...
                st.metric("Profitable Tokens", wallet.get('num_tokens_in_profit', 0))
...
if __name__ == "__main__":
    ...
    with st.sidebar.expander("❓ Help & Instructions"):
        ...
        **Note**: Make sure you have valid Helius and Birdeye API keys in your .env file.
        """)